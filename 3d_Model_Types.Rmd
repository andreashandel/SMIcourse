...
There is no single best kind of model, it depends on the question/scenario. Choosing the right model type for a given project is like choosing any tool or approach in science: Pick the one that's best and most suitable to answer the question. Choosing an appropriate model for a given task is part of the _art_ of good modeling, there is unfortunately no recipe. Of course, choosing the best model solely based on scientific considerations is the ideal idea. In practice, other considerations come into play. The expertise of the person building the model will play a role, so do feasibility (computation time, model complexity), â€œenvironmentâ€? (what approaches do others use), and â€œmarketingâ€? (what kind of models â€œsellâ€?). 
