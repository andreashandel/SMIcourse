...
```{r ibmexample21,  fig.cap='Example of a simple IBM model showing uninfected cells (green), infected cells (red) and virus (blue).',  echo=FALSE, out.width = "100%", fig.align='center'}
...
```{r disc,  fig.cap='Example of a discrete time model. Pretty much every IBM/ABM is updated in discrete time units.',  echo=FALSE, out.width = "80%", fig.align='center'}
knitr::include_graphics("./media/netlogo.png")
```
...
```{r ibmexample21spatial,  fig.cap='Simple 2d structure in an ABM. This might e.g. represent a section of epithelial tissue for an influenza virus infection.',  echo=FALSE, out.width = "60%", fig.align='center'}
knitr::include_graphics("./media/netlogo.png")
```
...
There is no single best kind of model, it depends on the question/scenario. Choosing the right model type for a given project is like choosing any tool or approach in science: Pick the one that's best and most suitable to answer the question. Choosing an appropriate model for a given task is part of the _art_ of good modeling, there is unfortunately no recipe. Of course, choosing the best model solely based on scientific considerations is the ideal idea. In practice, other considerations come into play. The expertise of the person building the model will play a role, so do feasibility (computation time, model complexity), “environment” (what approaches do others use), and “marketing” (what kind of models “sell”). 
