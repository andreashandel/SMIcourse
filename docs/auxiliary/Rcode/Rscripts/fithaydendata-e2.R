...
Vmin=0; #level of detection for virus in log10 units according to paper
...
parnames=c('V1', 'b','bp',  'dI', 'dV', 'p',  'e2' );
lb=c(V1low, blow,bplow, dIlow,dVlow, plow, e2low);
ub=c(V1high, bhigh,bphigh,dIhigh,dVhigh,phigh,  e2high);

loglb=log10(lb); logub=log10(ub); #fit in log space


#load previous best fit and save new best fit
filename.load=paste("hayden-fit-results-e2.Rdata",sep="");
filename.save=paste("hayden-fit-results-e2.Rdata",sep="");
...
timeseriesname = "haydenfit-timeseries-e2.Rdata"
